...
{pkgs, ...}: {
  ...
  # programs.plasma.startup.startupScript."pactl-subscribe" = let
  #   pactl = "${pkgs.pulseaudio}/bin/pactl";
  #   notify-send = "${pkgs.libnotify}/bin/notify-send";
  #   notifyVolumeScript = pkgs.writeShellScript "volume-notification" ''
  #     function get_volume() { ${pactl} get-sink-volume @DEFAULT_SINK@ | grep -oP '\d+%' | head -1 | cut -d'%' -f1; }
  #     function get_muted() { ${pactl} get-sink-mute @DEFAULT_SINK@ | awk '{print $2}'; }
  #     last_volume=$(get_volume)
  #     last_muted=$(get_muted)
  #     ${pactl} subscribe | while read -r event; do
  #       if echo "$event" | grep -q "Event 'change' on sink"; then
  #         volume=$(get_volume)
  #         muted=$(get_muted)
  #         if [[ $volume != $last_volume || $muted != $last_muted ]]; then
  #           if [[ $muted == "no" ]]; then
