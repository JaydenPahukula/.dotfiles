# kate config
{
  ...
  config = lib.mkIf config.programs.kate.enable {
    home.packages = [
      config.programs.kate.package
    ];

    # make kate the default text editor
    home.sessionVariables = {
      "EDITOR" = "${config.programs.kate.package}/bin/kate";
      "VISUAL" = "${config.programs.kate.package}/bin/kate";
    };
    xdg.mimeApps = {
      enable = true;
      defaultApplications = {
        "text/plain" = "${config.programs.kate.package}/bin/kate";
      };
    };

    # manually create configuration file
    # (plasma-manager module doesn't have a lot of config options)
    programs.plasma.configFile."katerc" = lib.mkForce {
      "General" = {
